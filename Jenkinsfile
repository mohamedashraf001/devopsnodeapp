...
pipeline {
    ...
    environment {
        IMAGE_NAME = "mohamedashraf001/dockernodeapp:latest"
        KUBECONFIG = "/home/jenkins/.kube/config" // تأكد من مسار kubeconfig الصحيح داخل WSL
    }
    ...
    stages {
        ...
        stage('Deploy to Kubernetes') {
            steps {
                script {
                    echo "Checking if deployment exists..."
                    def deploymentExists = sh(script: "wsl kubectl get deployment dockernodedepolyment --ignore-not-found -o jsonpath='{.metadata.name}'", returnStdout: true).trim()

                    if (!deploymentExists) {
                        echo "Creating new deployment..."
                        sh "wsl kubectl create deployment dockernodedepolyment --image=$IMAGE_NAME"
                    } else {
                        echo "Updating existing deployment..."
                        sh "wsl kubectl set image deployment/dockernodedepolyment dockernodedepolyment=$IMAGE_NAME --record"
                        sh "wsl kubectl rollout status deployment/dockernodedepolyment"
                    }

                    echo "Checking if service exists..."
                    def serviceExists = sh(script: "wsl kubectl get service dockernodeservice --ignore-not-found -o jsonpath='{.metadata.name}'", returnStdout: true).trim()
                    
                    if (!serviceExists) {
                        echo "Exposing deployment as a service..."
                        sh "wsl kubectl expose deployment dockernodedepolyment --name=dockernodeservice --type=LoadBalancer --port=3000"
                    }

                    echo "Fetching services status..."
                    sh 'wsl kubectl get services'
                }
            }
        }
    }
